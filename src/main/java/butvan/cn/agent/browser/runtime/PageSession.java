...
public class PageSession {
     ...
     */
    public void close() {
        if (context != null) { // 如果已经创建过 BrowserContext
            context.close(); // 关闭 Playwright 上下文，会同时关闭里面的页面
            context = null; // 清空 context 引用，避免重复使用已关闭对象
            currentPage = null; // 清空当前页面引用
            elementMap.clear(); // 清空元素索引缓存
        }
    }
